			...
			if component_editable then
				...
				elseif attached {CONF_CLUSTER} conf_notable as lt_cluster then
					if not lt_cluster.is_readonly then
						...
						l_date := l_system.file_date
						create l_entry.make ("Unnamed", Void, Void, Void, Void, Void, component_id, {EIS_ENTRY}.default_type, Void)
...
--							modify_entry_in_class (lt_entry, l_new_entry, lt_class)
			...
			if l_done then
				if attached system_of_conf_notable (conf_notable) as lt_system then
					storage.register_entry (l_new_entry, component_id, lt_system.file_date)
				end
			end
		end
